...
- (void)AlertInCurrentViewWithString:(NSString *)string {
  ...
  [MBProgressHUD showMessage:string view:[UIApplication sharedApplication].delegate.window];
}
...
- (void)changeVoiceImage {
  ...
  if ([_model.message isReceived]) {
    voiceImagePreStr = @"reciveVoice";
  } else {
    voiceImagePreStr = @"sendVoice";
  }
  _messageContent.voiceConent.image = [UIImage imageNamed:[NSString stringWithFormat:@"%@%zd", voiceImagePreStr, self.index % 2]];
  if (_isPlaying) {
    self.index++;
