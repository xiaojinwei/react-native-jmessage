...
- (void)viewDidLoad {
  ...
  [super viewDidLoad];
  [[NSUserDefaults standardUserDefaults]setObject:@"9" forKey:@"maxPhotoNum"];
...
- (XHVoiceRecordHUD *)voiceRecordHUD {
  if (!_voiceRecordHUD) {
    _voiceRecordHUD = [[XHVoiceRecordHUD alloc] initWithFrame:CGRectMake(0, 0, 193, 193)];
  }
...
//  browser.currentPhotoIndex = cell.model.photoIndex; // 弹出相册时显示的第一张图片是？
  ...
  [browser show];
//    DTCustomWebViewController *dt=[[DTCustomWebViewController alloc]initWithStartPage:@"index"];
