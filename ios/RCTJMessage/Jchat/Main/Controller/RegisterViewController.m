...
-(void)setRegisterUI
{
    ...
    [btn addTarget:self action:@selector(registerAccout) forControlEvents:UIControlEventTouchUpInside];
...
-(void)registerAccout
{
    ...
    [JMSGUser registerWithUsername:userName password:password completionHandler:^(id resultObject, NSError *error) {
        if (error==nil) {
            [MBProgressHUD showSuccess:@"注册成功" toView:self.navigationController.view];
