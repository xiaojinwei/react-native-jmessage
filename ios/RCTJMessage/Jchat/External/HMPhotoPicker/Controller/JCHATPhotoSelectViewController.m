...
- (void)didSelectStatusChange:(JCHATPhotoModel *)model :(id)sender{
     UIButton *selectBtn = (UIButton *)sender;
     selectBtn.selected = model.isSelected;
     
    NSString *maxNum=[[NSUserDefaults standardUserDefaults]objectForKey:@"maxPhotoNum"];
    int max=[maxNum intValue];
  if ([[[UIDevice currentDevice]systemVersion] floatValue]>= 8) {
    if (selectedPhotoDic[model.photoAsset] == nil) {
        
      if (selectedPhotoDic.count > max-1) {
        [MBProgressHUD showMessage:[NSString stringWithFormat:@"最多选择%d张图片",max] view:self.view];
    ...
    if (selectedPhotoDic[model.asset] == nil) {
      if (selectedPhotoDic.count > max-1) {
        ...
        [MBProgressHUD showMessage:[NSString stringWithFormat:@"最多选择%d张图片",max] view:[UIApplication sharedApplication].delegate.window];
        selectBtn.selected=NO;
        model.isSelected=NO;
        return;
      }
