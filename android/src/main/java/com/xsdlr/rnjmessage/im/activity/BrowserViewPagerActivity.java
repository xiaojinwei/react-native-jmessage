...
import com.xsdlr.rnjmessage.im.chatting.photoview.PhotoView;
...
import com.xsdlr.rnjmessage.im.view.ImgBrowserViewPager;
...
public class BrowserViewPagerActivity extends BaseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        long groupId = intent.getLongExtra(Contracts.GROUP_ID, 0);
        String targetAppKey = intent.getStringExtra(Contracts.TARGET_APP_KEY);
        mMessageId = intent.getIntExtra("msgId", 0);
        if (groupId != 0) {
            mConv = JMessageClient.getGroupConversation(groupId);
        } else {
            String targetId = intent.getStringExtra(Contracts.TARGET_ID);
            if (targetId != null) {
                mConv = JMessageClient.getSingleConversation(targetId, targetAppKey);
            }
        }
        mStart = intent.getIntExtra("msgCount", 0);
        mPosition = intent.getIntExtra(Contracts.POSITION, 0);
...
//        }
    ...
    private void initImgPathList() {
        mMsgIdList = this.getIntent().getIntegerArrayListExtra(Contracts.MsgIDs);
        Message msg;
        ImageContent ic;
    ...
    private OnClickListener listener = new OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            if (viewId == R.id.return_btn){
                ...
                setResult(Contracts.RESULT_CODE_SELECT_PICTURE, intent);
                finish();
                ...
                imgContent.downloadOriginImage(mMsg, new DownloadCompletionCallback() {
                    ...
                    public void onComplete(int status, String desc, File file) {
                        ...
                        if (status == 0) {
                            ...
                            bundle.putInt(Contracts.POSITION, mViewPager.getCurrentItem());
                            msg.setData(bundle);
                            msg.sendToTarget();
                        } else {
                            if (mProgressDialog != null) {
            ...
            if (activity != null) {
                switch (msg.what) {
                    case DOWNLOAD_ORIGIN_IMAGE_SUCCEED:
                        ...
                        activity.mPathList.set(bundle.getInt(Contracts.POSITION), bundle.getString("path"));
                    ...
                    case SEND_PICTURE:
                        ...
                        activity.setResult(Contracts.RESULT_CODE_BROWSER_PICTURE, intent);
                        activity.finish();
                        break;
                    //显示下载原图进度
