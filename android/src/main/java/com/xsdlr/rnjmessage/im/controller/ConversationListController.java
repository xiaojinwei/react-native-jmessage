...
import com.xsdlr.rnjmessage.im.adapter.ConversationListAdapter;
import com.xsdlr.rnjmessage.im.chatting.ChatActivity;
import com.xsdlr.rnjmessage.im.chatting.utils.DialogCreator;
import com.xsdlr.rnjmessage.im.tools.SortConvList;
import com.xsdlr.rnjmessage.im.view.ConversationListView;
    ...
    public void onItemClick(AdapterView<?> viewAdapter, View view, int position, long id) {
        ...
        try {
            if (position > 0) {
                ...
                intent.putExtra(Contracts.CONV_TITLE, conv.getTitle());
                if (null != conv) {
                    // 当前点击的会话是否为群组
                    if (conv.getType() == ConversationType.group) {
                        ...
                        intent.putExtra(Contracts.GROUP_ID, groupId);
                        intent.putExtra(Contracts.DRAFT, getAdapter().getDraft(conv.getId()));
                        intent.setClass(mContext.getActivity(), ChatActivity.class);
                        mContext.getActivity().startActivity(intent);
                        return;
                    } else {
                        String targetId = ((UserInfo) conv.getTargetInfo()).getUserName();
                        intent.putExtra(Contracts.TARGET_ID, targetId);
                        intent.putExtra(Contracts.TARGET_APP_KEY, conv.getTargetAppKey());
